...
<div class="row">
	<div class="col-xs-12 col-md-6">
		<div id="twocheckout_payment_module15" class="payment_module">
			<h3 class="twocheckout_title">
				<img id="secure-icon" alt="{l s='secure payment' mod='brinkscheckout'}" src="{$module_dir|escape:false}img/secure-icon.png" />{l s='Pay by credit card with our secured payment server' mod='brinkscheckout'}
			</h3>
			<div class="alert alert-danger" style="display:none" id="twocheckout_error_creditcard">
				<p>{l s='Payment Authorization Failed: Please verify your Credit Card details are entered correctly and try again, or try another payment method.' mod='brinkscheckout'}</p>
			</div>
			...
			<form action="{$link->getModuleLink('brinkscheckout', 'validation')|escape:false}" method="POST" id="twocheckoutCCForm" onsubmit="return false">
				...
				<br />
				<div class="block-left">
					<label>{l s='Expiration (MM/YYYY)' mod='brinkscheckout'}</label><br />
					<select id="expMonth" name="month" required>
						<option value="01">{l s='January' mod='brinkscheckout'}</option>
						<option value="02">{l s='February' mod='brinkscheckout'}</option>
						<option value="03">{l s='March' mod='brinkscheckout'}</option>
						<option value="04">{l s='April' mod='brinkscheckout'}</option>
						<option value="05">{l s='May' mod='brinkscheckout'}</option>
						<option value="06">{l s='June' mod='brinkscheckout'}</option>
						<option value="07">{l s='July' mod='brinkscheckout'}</option>
						<option value="08">{l s='August' mod='brinkscheckout'}</option>
						<option value="09">{l s='September' mod='brinkscheckout'}</option>
						<option value="10">{l s='October' mod='brinkscheckout'}</option>
						<option value="11">{l s='November' mod='brinkscheckout'}</option>
						<option value="12">{l s='December' mod='brinkscheckout'}</option>
					</select>
					<span> / </span>
				...
				<br />
				<div class="block-left">
					<label>{l s='CVC' mod='brinkscheckout'}</label><br />
					<input class="numeric" id="cvv" type="text" size="4" autocomplete="off" required />
				</div>
				<br />
				<input type="submit" class="button" id="submit_payment" {*onclick="retrieveToken()"*} value="{l s='Submit Payment' mod='brinkscheckout'}" />
